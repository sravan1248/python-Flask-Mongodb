...
def DeliveryConfirmationSubscription(subscription_id, o_id):
    data = mongo.db.OWO
    ...
    user_id = int()
    signin_type = str()
    for i in n_list.find({'notification_type': "Delivery Confirmation", 'status': True}):
        message_title = i['notification_title']
        m_body = i['notification_text_description']
        message_body = m_body.replace(str("{{order_id}}"), str(o_id))
        for s in db_ps.find({'subscription_id': subscription_id}):
            user_id = s['user_id']
                            ...
                            for v in db_wallet.find():
                                ...
                                if int(u_id) == int(w_u_id) and str(s_type) == str(w_s_type):
                                    if count == 1 and w_u_id == u_id and str(s_type) == str(w_s_type):
                                        count = count + 1
                                        ts = calendar.timegm(time.gmtime())
                                        transaction_id = 'EVENTREFUND'+ str(ts)
                            ...
                            for c in db_wallet.find():
                                ...
                                if int(u_id) == int(w_u_id) and str(s_type) == str(w_s_type):
                                    if count == 1 and w_u_id == u_id and str(s_type) == str(w_s_type):
                                        count = count + 1
                                        ts = calendar.timegm(time.gmtime())
                                        transaction_id = 'INSTANTREFUND' + str(ts)
                            ...
                            for v in db_wallet.find():
                                ...
                                if int(u_id) == int(w_u_id) and str(s_type) == str(w_s_type):
                                    if count == 1 and w_u_id == u_id and str(s_type) == str(w_s_type):
                                        count = count + 1
                                        ts = calendar.timegm(time.gmtime())
                                        transaction_id = 'SUBSCREFUND' + str(ts)
                                        current_balance = v['current_balance']
...
def get_companyId_basedComapnyName():
    ...
    company_name = request.json['company_name']
    output = []
    for i in db.find({'brand.active_status': True}):
        c_name = i['company_name']
...
def getSubscriptionByDate():
    ...
    for i in db.find({'is_subscribed': True, 'products.cart_status': "deactive",
                      "subscription_status": "active", 'payment_status': 'success'}):
        products = []
        count = 0
        customer_id = i['user_id']
                            ...
                            if qnt[0] >= 1:
                                ...
                                for l in db_p.find({'product_id': p_id}):
                                    ...
                                    output.append({'customer_id': i['user_id'],
                                                   ...
                                                   'product_id': j['product_id'],
                                                   'product_quantity': qnt[0], 'subscription_id': s_id,
                                                   'product_name': l['product_name']
                                                   })
            except KeyError or ValueError:
                pass
                        ...
                        try:
                            ...
                            for k in s_qnt:
                                if int(subscription_id) == int(i['subscription_id']):
                                    output.append(
                                        {'product_id': p_id, 'mon': k['Mon'], 'tue': k['Tue'], 'wed': k['Wed'],
                ...
                for s_id in db.find({'subscription_id': subscription_id}):
                    output.append(
                        {'customer_id': s_id['user_id'], 'mobile_number': s_id['mobile_number'],
                         'customer_type': s_id['signin_type'], 'subscription_id': j['subscription_id'],
                         'product_count': j['product_count'], 'email_id': s_id['email_id'],
                         'sub_total': j['total_price'], 'discount': j['discount'], 'gst': j['gst'],
                         'delivery_charges': s_id['delivery_charges'],
                         'total_price': int(j['total_price']) + int(s_id['delivery_charges']),
                         'order_id': j['order_id'], 'transaction_id': j['transaction_id'],
                         'delivery_address': s_id['delivery_address'],
                         'plan_starting_date': s_id['starting_date'], 'plan_expire_date': s_id['plan_expiry_date'],
                                            ...
                                            if str(m_type) == str(membership):
                                                ...
                                                for n in db_loyalty.find():
                                                    ...
                                                    if int(u_id) == int(uid) and str(stype) == str(s_type):
                                                        ...
                                                        # DeliveryConfirmationSubscription(subscription_id, o_id)
                                                        return jsonify({'status': True,
        ...
        if str(q_category) == str(question_category):
            ...
            for j in category:
                ...
                if int(q_id) == int(question_id):
                    ...
                    return jsonify(
                        ({'status': True, 'message': 'Status changed successfully', 'result': output}))
            ...
            for s_id in db.find({'subscription_id':subscription_id}):
                ...
                if delivery_status == 'pending':
                    output.append({'customer_id': s_id['user_id'], 'mobile_number': s_id['mobile_number'],
                                   ...
                                   'total_price': int(j['total_price']) + int(delivery_charges1),
                                   'order_id': j['order_id'], 'transaction_id': j['transaction_id'],
                                   'delivery_address': s_id['delivery_address'], 'plan_starting_date': s_id['starting_date'],
                                   'plan_expire_date': s_id['plan_expiry_date'], 'subscription_status': s_id['subscription_status'],
...
#         except KeyError or ValueError:
#             pass
#     return jsonify({'status': False, 'message': 'Please check your credentials', 'result': output})
    ...
    if trending_brand is False:
        for b in db.find():
            try:
                ...
                for j in brand:
                    b_name = j['brand_name']
                    if brand_name == b_name:
                        brand_id = j['brand_id']
                        output.append(
                            {'brand_name': brand_name, 'brand_id': brand_id, 'company_id': company_id,
                             'company_name': company_name})
                        return jsonify({'status': False, 'message': (
                                 " " + brand_name + " " + 'is already exist'),
                                        'result': output})
            except KeyError or ValueError:
                pass
        for b in db.find():
            try:
                ...
                output.append(
                    {'admin_id': admin_id,
                     ...
                     'company_name': company_name, 'company_id': company_id,
                     'brand_name': str(brand_name), 'order': int(order), 'trending_brand': trending_brand,
                     'brand_photo': "", 'brand_description': str(brand_description),
                     'date_of_creation': date_of_creation})
                return jsonify({'status': True, 'message': 'Brand added successfully',
                                'result': output})
            except KeyError or ValueError:
            ...
            for j in brand:
                ...
                if brand_name == b_name:
                    ...
                    output.append(
                        {'brand_name': brand_name, 'brand_id': brand_id, 'company_id': company_id,
                         'company_name': company_name})
                    return jsonify({'status': False, 'message': (brand_name + " " + 'is already exist'),
                                    'result': output})
        except KeyError or ValueError:
            pass
    brand_id = 1
    for b in db.find({'company_id': int(company_id)}):
        try:
            brand = b['brand']
            brand_id_list = [id['brand_id'] for id in brand]
            if len(brand_id_list) is 0:
...
def getMembership():
    ...
    for i in db_sub.find():
        ...
        for j in db.find():
            subscription_id = j['subscription_id']
            if int(s_id) == int(subscription_id):
                user_id = j['user_id']
                email_id = j['email_id']
                mobile_number = j['mobile_number']
        ...
        if int(u_id) == int(user_id) and str(s_type) == str(signin_type):
            for j in r_earning:
                ...
                if str(e_type) == 'subscribed':
                    ...
                    try:
                        temp['membership_type'] = j['membership_type']
                    except KeyError or ValueError:
                        temp['membership_type'] = ""
...
def getAllSubscription():
    ...
    output = []
    # date = request.json['date']
    for i in db.find({'subscription_status': "active", 'payment_status': 'success'}):
        try:
            prd = i['products']
            try:
            ...
            for k in prd:
                ...
                output.append({'subscription_id': i['subscription_id'], 'buy_plan': i['buy_plan'],
                               ...
                               'total_price': total_price + int(delivery_charges)})
        except KeyError or ValueError:
            pass
    res_list = {frozenset(item.items()): item for item in output}.values()
