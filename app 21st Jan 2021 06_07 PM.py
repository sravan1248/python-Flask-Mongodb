...
def PaymentSuccess(wallet_id):
    ...
    for j in db_c.find():
        for k in db_i.find():
            for l in j, k:
                if int(wallet_id) == int(l['wallet_id']):
                    user_id = l['user_id']
...
def AddingMoneyToWallet(wallet_id, amount):
    ...
    for i in n_list.find({'notification_type': "Adding Money to Wallet", 'status': True}):
        message_title = i['notification_title']
        m_body = i['notification_text_description']
        message_body = m_body.replace("{{amount}}", amount)
        print(message_body)
...
def UserSignupSuccessfulEmailCorporate(user_id, signin_type, official_email_id):
    data = mongo.db.OWO
    ...
    notifications.insert_one({'user_id': user_id, 'signin_type': signin_type, 'title': message_title,
                              'notifications': m_body,
                              'date_time': datetime.datetime.now()})


def UserSignupSuccessfulEmailIndividual(user_id, signin_type, email_id):
    data = mongo.db.OWO
    notifications = data.email_notifications
    n_list = data.email_notification_text
    message_title = str()
...
def PaymentSuccessEmail(wallet_id):
    data = mongo.db.OWO
    db_c = data.corporate_users
    db_i = data.individual_users
    notifications = data.email_notifications
...
def PaymentFailureEmail(wallet_id):
    ...
    result = {x['wallet_id']: x for x in e_id + official_e_id}.values()
    print(result)
    # push_notification(email_id, message_title, message_body)
    msg = Message(message_title, sender="ramadevig@fugenx.com", recipients=[email_id])
    msg.body = message_body
    print(msg)
    mail.send(msg)
...
def SubscriptionSuccessEmail(wallet_id):
    data = mongo.db.OWO
    db_c = data.corporate_users
    db_i = data.individual_users
    notifications = data.email_notifications
...
def UserSignupSuccessfullySMSCorporate(user_id, signin_type, contact_number):
    data = mongo.db.OWO
    notifications = data.SMS_notification_text
    n_list = data.sms_notification
    for i in notifications.find({'notification_type': "User Sign Up Successfull", 'status':True}):
        ...
        print(f)
        n_list.insert_one({'user_id': user_id, 'signin_type': signin_type, 'title': message_title,
                            'notifications': m_body,'date_time': datetime.datetime.now()})
...
def PaymentSuccessSMS(wallet_id):
    data = mongo.db.OWO
    db_c = data.corporate_users
    db_i = data.individual_users
    notifications = data.sms_notification
                                ...
                                if str(r_type) == 'Referral Points':
                                    ...
                                    for n in db_l.find():
                                        ...
                                        if int(id) == int(u_id) and str(stype) == str(s_type):
                                            ...
                                            UserSignupSuccessfulEmailCorporate(user_id, signin_type, official_email_id)
                                            UserSignupSuccessfullySMSCorporate(user_id, signin_type, mobile_number)
                                    ...
                                    UserSignupSucessfull(user_id, signin_type, first_name, firebase_id)
                                    UserSignupSuccessfulEmailCorporate(user_id, signin_type, official_email_id)
                                    UserSignupSuccessfullySMSCorporate(user_id, signin_type, contact_number)
                                    return jsonify(
                                ...
                                if str(r_type) == 'Referral Points':
                                    ...
                                    for n in db_l.find():
                                        ...
                                        if int(id) == int(u_id) and str(stype) == str(s_type):
                                            ...
                                            UserSignupSucessfull(user_id, signin_type, first_name, firebase_id)
                                            UserSignupSuccessfulEmailIndividual(user_id, signin_type, email_id)
        ...
        if user_wallet.find({'wallet_id': wallet_id}) is not None:
            for u in user_wallet.find({'wallet_id':wallet_id}):
                ...
                if total_amount > amount and u['current_balance']>amount:
                    ...
                    PaymentSuccess(wallet_id)
                    PaymentSuccessEmail(wallet_id)
                    PaymentSuccessSMS(wallet_id)
                     ...
                     if status == "success":
                         ...
                         try:
                            subscription_amount = i['subscription_amount']
                             ...
                             SubscriptionSuccessEmail(wallet_id)
...
def SubscriptionPlanPause():
    ...
    for i in db.find({'subscription_id':subscription_id,"subscription_status":"active"}):
        if i['starting_date'] <= t_date < i['plan_expiry_date']:
            db.update_many({'subscription_id':subscription_id},{'$set':{'subscription_status':subscription_status,'plan_pause_date':t_date}})
            output.append({'subscription_id':subscription_id,'subscription_status':subscription_status,'plan_start_date':i['starting_date'],'plan_expiry_date':i['plan_expiry_date'],'plan_pause_date':t_date})
            return jsonify({'status':True,'message':'you have successfully paused your subscription plan','result':output})
        else:
            return jsonify({'status': False, 'message': 'sorry you cannot modify your subscription plan as your plan has either not started or expired', 'result': output})
    else:
        for j in db.find({'subscription_id': subscription_id, "subscription_status": "pause"}):
                ...
                #a =  datetime.datetime.strptime(plan_start_day, "%Y-%m-%d")
                #b = datetime.datetime.strptime(plan_pause_day, "%Y-%m-%d")
                #c = abs((b-a).days)
                #print(c)
                #buy_plan = j['buy_plan']
                #actual_plan_frequency = buy_plan - c
                #print(actual_plan_frequency)
                #s_date = datetime.datetime.strptime(t_date, "%Y-%m-%d")
                #modified_date = s_date + timedelta(days=actual_plan_frequency)
                plan_expiry_date = j['plan_expiry_date']
                print(plan_expiry_date)
                db.update_many({'subscription_id': subscription_id},
                               {'$set': {'subscription_status': subscription_status,'plan_activation_date':t_date,'plan_expiry_date':plan_expiry_date,'buy_plan':j['buy_plan']}})
                output.append({'subscription_id': subscription_id, 'subscription_status': subscription_status,
                               'plan_start_date': j['starting_date'], 'plan_expiry_date': plan_expiry_date})
...
#-------------------------------------------------- Download Invoice ---------------------------------------------------
@app.route('/owo/download_invoice', methods=['POST'])
@jwt_required
def DownloadInvoice():
    data = mongo.db.OWO
    db = data.event_management
...
def appGetFAQCategory():
    data = mongo.db.OWO
    db = data.FAQ_management
    output = []
    for i in db.find():
        output.append({'question_category': i['question_category'], 'category': i['category']})
    return jsonify({'status': True, 'message': 'all question category data successfully', 'result': output})
        ...
        if str(city) == str(city_name):
            try:
                ...
                for j in p_type:
                    ...
                    output.append({'product_id': i['product_id'], 'product_name': i['product_name'],
                                   'product_image': i['product_image'], 'available_quantity': i['product_quantity'],
                                   'brand_name': i['brand_name'], 'company_name': i['company_name'],
...
def setDefaultCancelOrders():
    ...
    if str(signin_type) == 'corporate':
                        ...
                        if int(a_id) == int(address_id):
                            if d_address == True:
                                ...
                                db.update_many({'user_id': int(user_id), 'user_address.address_id': int(address_id)},
                                               {'$set': {'user_address.$.default_address': True}})
                                output.append({'user_id': user_id, 'address_id': address_id, 'default_address': True})
                                for s_id in db5.find({'user_id': user_id, 'signin_type': signin_type}):
                                    subscription_id = s_id['subscription_id']
                                    print(subscription_id)
                                    ...
                                    print("ok_sub")
                                for k in db2.find({'user_id': user_id, 'signin_type': signin_type,
                                                       'order_status': "confirmed"}):
                                        event_id = k['event_id']
                                        total_amount = int(k['sub_total'])
                                        db2.update_many({'event_id': event_id}, {'$set': {'order_status': "cancelled"
                                                                                          ,'delivery_status':"cancelled"}})
                                for l in db3.find({'user_id': user_id, 'signin_type': signin_type,
                                                           'order_status': "confirmed"}):
                                            instant_id = l['instant_id']
                                            instant_amount = int(l['sub_total'])
                                            db3.update_many({'instant_id': instant_id},
                                                            {'$set': {'order_status': "cancelled"
                                                                      ,'delivery_status':"cancelled"}})
                                for m in db4.find({'subscription_id': subscription_id}):
                                                ...
                                                db4.update_many({'wallet_id': wallet_id}, {
        ...
        for ind in db1.find({'user_id':user_id,'signin_type':signin_type}):
            u_id = ind['user_id']
            if int(u_id) == int(user_id):
                print("ok user")
                try:
